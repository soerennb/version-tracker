...
- Required merge checks are CI gate and Security gate from the active Protect master repository ruleset; classic branch protection is not the source of truth. Tags matching v0.*.* must be based on master and run validation, build checksummed Docker deployment and Dockerless native bundles with release metadata, publish GHCR images with provenance/SBOM, smoke-test and scan the immutable digest through the complete Compose stack, and generate release notes; the release workflow does not deploy the application.
