<template>
    <section class="mx-auto max-w-6xl space-y-8">
        ...
        <template v-else>
            ...
            <CompareSection :title="$t('release.composition')" :left="comparison.left.composition" :right="comparison.right.composition"><template #default="{ value }"><div v-if="value" class="space-y-3 text-sm"><p><span class="text-slate-500">{{ $t('release.baseline') }}</span> <strong class="font-mono text-slate-950">{{ value.baseline.version }}</strong></p><p><span class="text-slate-500">{{ $t('release.eformsComponent') }}</span> <strong class="font-mono text-slate-950">{{ value.eforms_component.version }}</strong></p><p><span class="text-slate-500">{{ $t('release.activeEformsSdk') }}</span> <strong class="font-mono text-slate-950">{{ value.active_eforms_sdk.version }}</strong></p><p v-for="item in value.supported_interfaces" :key="item.id"><span class="text-slate-500">{{ item.name }}</span> <strong class="font-mono text-slate-950">{{ item.version }}</strong></p></div><p v-else class="text-sm text-slate-500">{{ $t('release.notTracked') }}</p></template></CompareSection>
